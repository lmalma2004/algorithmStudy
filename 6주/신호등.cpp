...
public:
    ...
    Node(int n, int pn, long long t, long long st) {
        node = n;
        preNode = pn;
        time = t;
        signalTime = st;
    ...
    void updateTime() {
        if (preNode == -1)
            return;
        while (!canMove()) {
            time = ((time / signalTime) + 1) * signalTime; // 여기부분이 어디냐
        }
    }
};
Node nodes[100001];
...
long long solve() {
    ...
    while (!pq.empty()) {
        ...
        Visit visit(curr.preNode, curr.node);
